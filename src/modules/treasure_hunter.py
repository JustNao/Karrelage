...
class TreasureHunter(DofusModule):
    ...
    def move(self):
        ...
        if (not self.botting) or (
            self.hint_position.distance > 10 and not self.phorreur["lookingFor"]
        ):
            return
    ...
    def handle_TreasureHuntMessage(self, packet):
        ...
        if (
            ...
            if self.hint_position.distance == 666:
                ...
                print(
                    f"{client_hint_name} found {self.hint_position.distance} maps at {self.hint_position.__str__()}"
                )
                self.step_update()
                self.move()
        else:
    ...
    def reset(self):
        self.hint_position.distance = 666
        self.direction = "stay"
        self.phorreur["lookingFor"] = False
        self.autopilotMoving = False
